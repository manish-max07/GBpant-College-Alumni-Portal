 ...
 * For users who have created an account (set password) but haven't completed their profile details
 */
const sendProfileCompletionReminderEmail = async (email, fullName, userType = 'member') => {
  if (!email) {
    console.error('❌ Missing email for profile completion reminder');
    return false;
  }

  const transporter = createSMTPTransporter();
  if (!transporter) {
    console.error('❌ SMTP transporter not available for profile completion email');
    return false;
  }
  ...
  try {
    const info = await transporter.sendMail({
      from: `"GB Pant Alumni Portal" <${process.env.SMTP_USER}>`,
      to: email,
      subject: `Sorry: Your account isn't created yet — complete your profile to activate your GB Pant Alumni account`,
      html,
    ...
    console.error('❌ Failed to send profile completion reminder:', error.message);
    return false;
  }
};
